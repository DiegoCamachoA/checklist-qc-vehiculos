...
Aplicación Streamlit para generar checklists de control de calidad
a partir de Bases Técnicas (TDR) y Ofertas Técnicas en PDF.
"""
...
<style>
  ...
  .stDownloadButton > button:hover {
    background: rgba(58,142,255,0.12) !important;
    box-shadow: 0 0 12px rgba(58,142,255,0.2) !important;
  }
  ...
  .cl-item {
    ...
    gap: 10px;
    padding: 7px 8px;
    border-bottom: 1px solid rgba(46,51,74,0.6);
    font-size: 0.85rem;
    color: var(--text);
  }
  ...
  .cl-cat   { font-family: var(--mono); font-size: 0.7rem; color: var(--accent2); background: rgba(58,142,255,0.1); padding: 1px 6px; border-radius: 3px; }
...
<div class="app-header">
  ...
  <div>
    ...
    <span class="badge">IA · Claude API</span>
...
with st.sidebar:
    ...
    api_key_input = st.text_input(
        ...
        type="password",
        help="Si no tienes un .env, ingresa tu API key aquí.",
        placeholder="sk-ant-..."
    )
    if api_key_input:
    ...
    4. **Descarga** el HTML para imprimir
    """)
    st.markdown("---")
...
with col_left:
    ...
    if uploaded_files:
        ...
        for f in uploaded_files:
            ...
            st.markdown(
                f'<span class="info-pill">📄 {f.name} &nbsp;·&nbsp; {size_kb} KB</span>',
                unsafe_allow_html=True
            )
    else:
        st.markdown(
            '<div class="status-warn">⚠ Ningún archivo cargado</div>',
            unsafe_allow_html=True
        )
    ...
    if not api_ready:
        st.markdown(
            '<div class="status-err">✗ Falta ANTHROPIC_API_KEY · Configura en el sidebar</div>',
            unsafe_allow_html=True
        )
    ...
    # ── Métricas (si hay checklist generado) ──
    ...
    with col_right:
        with st.spinner("Extrayendo texto de PDFs..."):
            try:
                ...
                st.markdown(
                    f'<div class="status-ok">✓ Texto extraído · {char_count:,} caracteres de {len(uploaded_files)} archivo(s)</div>',
                    unsafe_allow_html=True
                )
            except Exception as e:
                st.markdown(
                    f'<div class="status-err">✗ Error leyendo PDFs: {e}</div>',
                    unsafe_allow_html=True
                )
                st.stop()

        with st.spinner("Analizando con IA · Claude Sonnet..."):
            try:
                ...
                st.markdown(
                    '<div class="status-ok">✓ Checklist generado exitosamente</div>',
                    unsafe_allow_html=True
                )
            except json.JSONDecodeError as e:
                st.markdown(
                    f'<div class="status-err">✗ Error parseando respuesta del LLM: {e}</div>',
                    unsafe_allow_html=True
                )
                st.stop()
            except Exception as e:
                st.markdown(
                    f'<div class="status-err">✗ Error en API: {e}</div>',
                    unsafe_allow_html=True
                )
                st.stop()
...
with col_right:
    if "checklist_data" in st.session_state and st.session_state.checklist_data:
        ...
        with ex_col1:
            filename = f"checklist_qc_{datetime.now().strftime('%Y%m%d_%H%M')}.html"
            st.download_button(
            ...
            for cat, items in cats.items():
                ...
                for item in items:
                    ...
                    <div class="cl-item">
                      ...
                      <div style="flex:1">
                        <span class="cl-label">{item.get('item','')}</span>
                        <span class="cl-spec"> — {item.get('especificacion','')}</span>
                      </div>
                      <div style="width:80px;text-align:center;font-family:var(--mono);font-size:0.7rem;color:var(--muted)">SÍ / NO / OBS</div>
                    </div>
                    """, unsafe_allow_html=True)
        else:
            st.markdown('<div class="cl-spec" style="padding:8px">No se encontraron ítems de verificación física.</div>', unsafe_allow_html=True)

        # SECCIÓN 3: Accesorios Adicionales
        ...
        if accesorios:
            for acc in accesorios:
                ...
                <div class="cl-item">
                  <div class="cl-check"></div>
                  <div style="flex:1">
                    <span class="cl-label">{acc.get('item','')}</span>
                    <span class="cl-spec"> — {acc.get('descripcion','')}</span>
                  </div>
                  <div style="width:50px;text-align:center;font-family:var(--mono);font-size:0.7rem;color:var(--muted)">✓ / ✗</div>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.markdown('<div class="cl-spec" style="padding:8px">No se identificaron accesorios adicionales.</div>', unsafe_allow_html=True)
        ...
        if pruebas:
            for p in pruebas:
                ...
                st.markdown(f"""
                <div class="cl-test">
                  <div class="cl-test-name">{p.get('prueba','')}{ref_text}</div>
                  <div class="cl-test-line">Resultado ({p.get('unidad','')}): _________________________________</div>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.markdown('<div class="cl-spec" style="padding:8px">No se identificaron pruebas de funcionamiento.</div>', unsafe_allow_html=True)

        # SECCIÓN 5: Firmas
        st.markdown('<div class="cl-section-title">§ 5 · FIRMAS Y APROBACIÓN</div>', unsafe_allow_html=True)
        st.markdown("""
        <div style="display:flex; gap:32px; padding:16px 8px;">
          <div style="flex:1; text-align:center;">
            <div style="border-top:1px solid var(--border); padding-top:8px; margin-top:48px;"></div>
            <div style="font-family:var(--mono); font-size:0.78rem; color:var(--muted);">INGENIERO INSPECTOR</div>
            <div style="font-size:0.8rem; color:var(--text); margin-top:4px;">Nombre / CIP</div>
          </div>
          <div style="flex:1; text-align:center;">
            <div style="border-top:1px solid var(--border); padding-top:8px; margin-top:48px;"></div>
            <div style="font-family:var(--mono); font-size:0.78rem; color:var(--muted);">JEFE DE TALLER · CARROCERO</div>
            <div style="font-size:0.8rem; color:var(--text); margin-top:4px;">Nombre / DNI</div>
          </div>
        </div>
        """, unsafe_allow_html=True)

        st.markdown('</div>', unsafe_allow_html=True)  # close checklist-preview
        ...
        <div style="
          ...
          padding: 80px 40px;
          color: #7a8099;
          border: 2px dashed #2e334a;
          border-radius: 8px;
          background: #1a1d27;
        ">
          <div style="font-size: 3rem; margin-bottom: 16px;">📋</div>
          <div style="font-family: 'IBM Plex Mono', monospace; font-size: 0.9rem; margin-bottom: 8px; color: #e8eaf0;">
            Checklist pendiente de generación
          </div>
          <div style="font-size: 0.82rem;">
            Sube tus PDFs (TDR / Oferta Técnica) y presiona <strong>Generar Checklist</strong>
          </div>
        </div>
        """, unsafe_allow_html=True)
